 ...
 * spreads behind them are not.** Announcing "God catch" (3+ near-perfect IVs)
 * or a full bond heart (a near-capped EV pool) to the whole pack is deliberate
 * — that is what the Showcase is for. Withholding the spread itself is equally
 * deliberate. Publish the tier enum and nothing finer: `summarizeIvs().headline`
 * and friends name exact stats and belong behind `showCompetitiveDetails`.
 *
