 ...
 *
 * The static randomizer shares that RNG and is emitted here too:
...
/** Pinned upstream revision. Bump deliberately and regenerate. */
const SOURCE_REF = "4f0a494ea9d282fc034e61f1df363cf3a321384c";
const TARBALL_URL =
  `https://codeload.github.com/resetes12/pokeemerald/tar.gz/${SOURCE_REF}`;
...
function readEvoSlots(src, speciesIds) {
  ...
  // which means "roll from the first-stage pool", not "leave alone". Size from
  // both SPECIES_EGG and the highest assignment so shifted tail entries
  // (Farigiraf onward, Kleavor) are never dropped when the constant is low.
  const maxIndex = Math.max(...assignments.map((a) => a.index));
  const size = Math.max(speciesIds.get("SPECIES_EGG") ?? 0, maxIndex) + 1;
...
function main() {
  ...
  writeFileSync(
    ...
    [
      ...
      `// (Modern Emerald @ ${SOURCE_REF}). Do not edit by hand — run \`npm run data:randomizer\`.`,
      "//",
